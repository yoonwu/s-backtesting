...
select,input[type=date]{width:100%;background:var(--card2);color:var(--ink);border:1px solid var(--line);
...
<div class="card"><div class="eyebrow">파라미터</div>
 ...
 <div class="btnrow">
  ...
  <button onclick="maScan()">MA 50~300 전부 스캔</button>
  <button onclick="preset()">운영값으로 리셋 (MA150 · 32/36 · ±0.5%)</button>
 </div>
</div>
...
['ma','vin','vout','buf'].forEach(k=>$('p-'+k).oninput=()=>{$('o-'+k).textContent=$('p-'+k).value});
